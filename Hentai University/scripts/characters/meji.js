...
function writeEncounter(name) { //Plays the actual encounter.
	...
	switch (name) {
		...
		case "meji11" : {
			if(!galleryCheck('mejiAlt1')){
				...
				if(checkFlag('succubus', 'newCorruption') && !checkFlag('meji','corrupt')){
					...
					writeSpeech("player","","I actually already turned in the paperwork. Like I said, not too much is likely to change since we're already keeping your schedule fairly open, but...");
		...
		case "meji12" : {
			...
			writeText("There seems to be something on his mind... And you have a gut feeling it's something serious.");
			if(checkFlag('succubus', 'newCorruption') && !checkFlag('meji','corrupt')){
				writeText("You could probably bring up the corruption first, though...");
				writeFunction("writeEncounter('mejiCorruptionPrompt')", "Talk to mejiF about corruption");
			}
			writeFunction("writeEncounter('mejiCorruptionPrompt')", "Talk to mejiF about corruption");
			break;
